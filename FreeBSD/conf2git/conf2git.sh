...
# v1.7.0 — Portable self-lock + smart remote sync + **CLI-only self-update** + report + log rotation + retries
# Author: Karim Mansur <karim.mansur@outlook.com>
# Changelog: 1.7.0 makes self-update strictly CLI-triggered (no background/network access on normal runs).
#
# NOTE: Comments are in English by the author's preference.
set -eu
umask 022
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/bin"

CONF2GIT_VERSION="1.7.0"

# Preserve argv before parsing (for potential self-update re-exec)
CONF2GIT_ORIG_ARGS="${*:-}"
...
DRYRUN=false
FORCE_UPDATE=false          # set true by --self-update / --self-update-only
SELF_UPDATE_ONLY=false      # exit right after self-update when true
REPORT=false
...
log(){
  ...
  echo "$msg"
  [ -n "${LOGFILE:-}" ] && { echo "$msg" >>"$LOGFILE" 2>/dev/null || true; }
}
...
rotate_logs_if_needed(){
  ...
  while [ $i -ge 2 ]; do
    p=$((i-1))
    [ -f "$LOGFILE.$p.gz" ] && mv -f "$LOGFILE.$p.gz" "$LOGFILE.$i.gz" || true
    i=$((i-1))
  done
...
resolve_script_path(){
  sp="$0"
  case "$sp" in */*) :;; *) sp=$(command -v -- "$0" 2>/dev/null || echo "$0");; esac
...
self_update_check(){
  # Only called when FORCE_UPDATE=true (CLI-only since v1.7.0)
  [ "${CONF2GIT_UPDATED:-}" = "1" ] && { log "Self-update: already updated"; return 0; }
  [ -n "${UPDATE_URL:-}" ] || { log "Self-update: requested, but UPDATE_URL not set"; return 1; }

  SCRIPT_PATH="$(resolve_script_path)"
  [ -r "$SCRIPT_PATH" ] || { log "Self-update: unreadable script ($SCRIPT_PATH)"; return 1; }
  ...
  if [ -n "${EXPECTED_SHA256:-}" ]; then
    [ "$(sha256_file "$tmp")" = "$EXPECTED_SHA256" ] || { log "Self-update: hash mismatch"; rm -f "$tmp"; return 1; }
  fi

  old="$(sha256_file "$SCRIPT_PATH" 2>/dev/null || echo none)"
  new="$(sha256_file "$tmp" 2>/dev/null || echo none)"
  [ "$old" = "$new" ] && { log "Self-update: up to date"; rm -f "$tmp"; $SELF_UPDATE_ONLY && exit 0; return 0; }

  [ -w "$SCRIPT_PATH" ] || { log "Self-update: not writable ($SCRIPT_PATH)"; rm -f "$tmp"; $SELF_UPDATE_ONLY && exit 1; return 1; }
  ts=$(date +%Y%m%d%H%M%S)
  bak="${SCRIPT_PATH}.bak.$ts"
  cp -p "$SCRIPT_PATH" "$bak" || { log "Self-update: backup failed"; rm -f "$tmp"; $SELF_UPDATE_ONLY && exit 1; return 1; }
...
# Defaults for new options (can be overridden in cfg)
ALIGN_MODE="${ALIGN_MODE:-$ALIGN_MODE_DEFAULT}"
LOCK_BLOCKING="${LOCK_BLOCKING:-no}"   # "yes" or "no"
GIT_RETRY_MAX="${GIT_RETRY_MAX:-3}"    # integer >=1
PUSH_ENABLED="${PUSH_ENABLED:-yes}"    # "yes" or "no"
GIT_MARK_SAFE="${GIT_MARK_SAFE:-no}"   # "yes" or "no"

# Warn about deprecated AUTO_UPDATE usage (since v1.7.0)
if [ -n "${AUTO_UPDATE:-}" ] || [ -n "${UPDATE_URL:-}" ]; then
  if [ "$FORCE_UPDATE" = false ]; then
    log "Notice: Self-update is CLI-only since v1.7.0; AUTO_UPDATE/UPDATE_URL are ignored unless --self-update* is used."
  fi
fi

# Rotate logs, then (optionally) self-update
rotate_logs_if_needed
...
log "Start conf2git v${CONF2GIT_VERSION} on ${OS}; ALIGN_MODE=${ALIGN_MODE}; LOCK_BLOCKING=${LOCK_BLOCKING}; DRYRUN=${DRYRUN}"

# Portable self-lock (configurable blocking behavior)
if [ "${__CONF2GIT_LOCKED:-}" != "1" ]; then
  d="$(dirname -- "${LOCKFILE}")"; [ -d "$d" ] || mkdir -p "$d" || true
  if [ "$OS" = "freebsd" ] && command -v /usr/bin/lockf >/dev/null 2>&1; then
    if [ "${LOCK_BLOCKING}" = "yes" ]; then LK_OPTS="-k"; else LK_OPTS="-t 0"; fi
    log "Lock: using lockf (blocking=${LOCK_BLOCKING}) on ${LOCKFILE}"
    export __CONF2GIT_LOCKED=1; exec /usr/bin/lockf $LK_OPTS "${LOCKFILE}" "$0" "$@"
    ...
    if [ "${LOCK_BLOCKING}" = "yes" ]; then LK_OPTS=""; else LK_OPTS="-n"; fi
    log "Lock: using flock (blocking=${LOCK_BLOCKING}) on ${LOCKFILE}"
    export __CONF2GIT_LOCKED=1; exec "$FLOCK_BIN" $LK_OPTS "${LOCKFILE}" "$0" "$@"
  else
    LOCKDIR="${LOCKFILE}.d"
    mkdir "$LOCKDIR" 2>/dev/null || { log "Another run is active."; exit 0; }
    trap 'rmdir "$LOCKDIR" 2>/dev/null || true' EXIT INT TERM
    export __CONF2GIT_LOCKED=1
  fi
fi

# Helper: retry wrapper for flaky git operations
git_try(){
  i=1; max="${GIT_RETRY_MAX:-3}"
  while :; do
    if "$@"; then return 0; fi
    rc=$?
    [ "$i" -ge "$max" ] && return "$rc"
...
if [ ! -d "$REPO_ROOT/.git" ]; then
  ...
  cd "$REPO_ROOT"
  [ "${GIT_MARK_SAFE}" = "yes" ] && git config --global --add safe.directory "$REPO_ROOT" || true
  ...
  if git sparse-checkout list >/dev/null 2>&1; then
    log "Sparse-checkout already enabled. Setting path: $TARGET_PATH"; git sparse-checkout set "$TARGET_PATH"
  else
    log "Enabling sparse-checkout and restricting to: $TARGET_PATH"
    git sparse-checkout init --cone; git sparse-checkout set "$TARGET_PATH"
  fi
  log "Updating refs (ALIGN_MODE=${ALIGN_MODE})"
  if ! git_try git fetch --prune; then log "Warning: fetch failed after retries"; fi
...
if ! $DRYRUN; then
  ...
  log "Commit: $msg"; git commit -m "$msg"
  if [ "${PUSH_ENABLED}" = "yes" ]; then
    log "Pushing to '$DEFAULT_BRANCH'"
    if ! git_try git push origin "$DEFAULT_BRANCH"; then
      log "Error: push failed after retries"; exit 1
    fi
  else
    log "Push disabled (PUSH_ENABLED=no). Skipping push step."
  fi
  log "Done."
...
if $REPORT; then
  ...
  echo "Changes: rsync_itemized=$CH"
  echo "Committed: ${REPORT_COMMITTED:-no} ${REPORT_COMMIT_SHA:+(sha $REPORT_COMMIT_SHA)}"
  echo "=================================================="
